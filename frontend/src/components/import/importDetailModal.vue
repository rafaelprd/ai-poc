  ...
  <Teleport to="body">
    <Transition name="modalFade">
      <div
        ...
        role="dialog"
        aria-modal="true"
        :aria-label="detail ? `Import #${detail.id} details` : 'Import details'"
        ...
        <div class="modal importDetailModal">
          ...
          <div class="modalBody stack">
            ...
            <div v-if="loading" class="importDetailLoading">
              ...
              <span class="helperText">Loading import details…</span>
            </div>
            ...
            >
              ...
              <Button variant="ghost" size="sm" type="button" @click="fetchDetail">
                Retry
              </Button>
            </div>
            ...
            <template v-else-if="detail">
              ...
              <div class="kpiRow">
                <article class="kpiCard">
                  <p class="kpiValue">{{ detail.totalFiles }}</p>
                  <p class="kpiLabel">Files</p>
                </article>
                <article class="kpiCard">
                  <p class="kpiValue">{{ detail.newTransactions }}</p>
                  <p class="kpiLabel">New txns</p>
                </article>
                <article class="kpiCard">
                  <p class="kpiValue">{{ detail.duplicateTransactions }}</p>
                  <p class="kpiLabel">Duplicates</p>
                </article>
                <article
                  class="kpiCard"
                  :class="{ kpiCardAlert: detail.failedFiles > 0 }"
                >
                  <p class="kpiValue">{{ detail.failedFiles }}</p>
              ...
              <div class="importDetailMeta">
                <div class="importDetailMetaItem">
                  <span class="importDetailMetaLabel">Account</span>
                  <span class="importDetailMetaValue">
                    {{ detail.accountId || '—' }}
                  </span>
                </div>
                <div class="importDetailMetaItem">
                  <span class="importDetailMetaLabel">Created</span>
                  <span class="importDetailMetaValue">{{ formatDate(detail.createdAt) }}</span>
                </div>
                <div class="importDetailMetaItem">
                  <span class="importDetailMetaLabel">Last updated</span>
                  <span class="importDetailMetaValue">{{ formatDate(detail.completedAt ?? detail.createdAt) }}</span>
                </div>
                <div class="importDetailMetaItem">
                  <span class="importDetailMetaLabel">Completed files</span>
                  <span class="importDetailMetaValue">{{ detail.totalFiles - detail.failedFiles }}</span>
                </div>
              </div>

              <!-- File list -->
              <div class="stackSm">
                <p class="fieldLabel">Files ({{ detail.files.length }})</p>
                ...
                <div v-else class="emptyState">
                  ...
                  <p class="emptyStateText">This import has no associated files.</p>
                </div>
              </div>
            </template>
          </div>
