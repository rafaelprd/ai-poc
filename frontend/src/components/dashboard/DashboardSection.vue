...
const dashboardIsEmpty = computed(() => {
  ...
  const noExpenses =
    !m ||
    (m.total_expenses === 0 && (!m.categories || m.categories.length === 0))
  ...
  const noTimeSeries =
    ...
    (!mo || (mo.cumulative_income ?? 0) === 0 && (mo.cumulative_expenses ?? 0) === 0)
...
const tableCategories = computed(() => {
  ...
  if ((c.uncategorized_total ?? 0) > 0 || (c.uncategorized_percentage ?? 0) > 0) {
    rows.push({
      category_id: null,
    ...
    <div v-else class="grid">
      ...
      <div class="row2">
        <div class="panel">
          <categoryPieChart
            ...
            :uncategorized-total="categoryBreakdown?.uncategorized_total ?? 0"
            :uncategorized-percentage="categoryBreakdown?.uncategorized_percentage ?? 0"
          />
        </div>
      ...
      <div class="rowFull">
        <spendingTimeSeriesChart
          :daily-data-points="(dailyTimeSeries?.data_points ?? []) as any"
          :monthly-data-points="(monthlyTimeSeries?.data_points ?? []) as any"
      ...
      <div class="rowBottom">
        ...
        <upcomingPaymentsList :payments="cardTracking?.upcoming_payments ?? []" />
      </div>

      <div v-if="error" class="errorBar" role="alert" aria-live="assertive">
        <strong>Erro:</strong>
        {{ error.message }}
      </div>
    </div>
