...
const dailyAvailable = computed(
    () =>
        Array.isArray(props.dailyDataPoints) &&
        props.dailyDataPoints.length > 0,
);
const monthlyAvailable = computed(
    () =>
        Array.isArray(props.monthlyDataPoints) &&
        props.monthlyDataPoints.length > 0,
);

const toggleVisible = computed(
    () => dailyAvailable.value && monthlyAvailable.value,
);
...
const activePoints = computed<TimeSeriesPoint[]>(() => {
    const list =
        mode.value === "daily"
            ? props.dailyDataPoints
            : props.monthlyDataPoints;
    ...
    return [...list]
        ...
        .sort(
            (a, b) =>
                parseDateISO(a.date).getTime() - parseDateISO(b.date).getTime(),
        );
});
    ...
    <section class="chartWrap" aria-label="Tempo gastos e receitas">
        <div class="headerRow">
            ...
            <div
                ...
                class="segmented"
                role="tablist"
                aria-label="Alternar granularidade"
            >
                <button
                    type="button"
                    class="segBtn"
